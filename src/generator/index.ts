...
export function generateSquad(options: GenerateOptions): GenerateResult {
  ...
  files.push({ path: join(squadDir, 'eval.mjs'), content: generateEvalScript(preset), label: '.squad/eval.mjs', kind: 'structural' });
  files.push({ path: join(squadDir, 'SCORECARD.md'), content: generateScorecard(preset), label: '.squad/SCORECARD.md', kind: 'content' });
...
function generateEvalScript(arch: Preset): string {
  const agents = arch.agents.map(a => a.name.toLowerCase());
  const agentJson = JSON.stringify(agents);
  const presetName = arch.displayName;
  const routingPatterns = arch.routing.rules.map(r => ({
    agent: r.agent.toLowerCase(),
    pattern: r.pattern,
  }));
  const routingJson = JSON.stringify(routingPatterns);

  return `#!/usr/bin/env node
// Squad Self-Check — validates squad integrity
// Generated by snap-squad for "${presetName}"
...
function generateScorecard(arch: Preset): string {
  // Generate routing test prompts from actual routing rules
  const routingRows = arch.routing.rules.map((r, i) =>
    `| ${i + 1} | "${r.description.split(',')[0].trim()}" | ${r.agent} | |`
  ).join('\n');

  // Generate dispatch combos based on available agents
  const hasDevRel = arch.agents.some(a => /doc|devrel/i.test(a.role));
  const hasTester = arch.agents.some(a => /test|qa/i.test(a.role));
  const hasScribe = arch.agents.some(a => /historian|journalist/i.test(a.role));
  const hasCoder = arch.agents.some(a => /core dev|full-stack|backend/i.test(a.role));

  const dispatchRows: string[] = [];
  if (hasCoder && hasDevRel) dispatchRows.push('| 1 | "Add a new feature and update the docs" | Coder + DevRel | |');
  if (hasCoder && hasTester) dispatchRows.push(`| ${dispatchRows.length + 1} | "Refactor this module and make sure tests pass" | Coder + Tester | |`);
  if (hasScribe) dispatchRows.push(`| ${dispatchRows.length + 1} | "Ship this milestone and capture the story" | Lead + Scribe | |`);
  ...
  for (const agent of arch.agents) {
    const role = agent.role.toLowerCase();
    const checks: string[] = [];
    if (/doc|devrel/i.test(role)) {
      checks.push('- [ ] Output uses direct imperative tone (not "you might want to consider...")');
    ...
    } else if (/historian|journalist/i.test(role)) {
      checks.push('- [ ] Journal entry uses timestamped table format');
      checks.push('- [ ] Level-Up moments captured');
      checks.push('- [ ] Honest about failures/pivots');
      checks.push('- [ ] Quotes human steering commands');
    } else if (/eval|quality/i.test(role)) {
      checks.push('- [ ] Eval baselines reviewed after prompt changes');
      checks.push('- [ ] Quality metrics reported');
    }
    if (checks.length > 0) {
      qualityChecks.push(`### ${agent.name} (${agent.role})\n\n${checks.join('\n')}`);
    }
  }

  return `# Eval Scorecard — ${arch.displayName}
